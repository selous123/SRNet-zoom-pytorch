...
plt.subplot(121)
plt.plot(axis, loss_log.numpy(),color = 'black' ,label= "EDSR_baseline")
plt.plot(axis, loss_b_log.numpy(),color = 'yellow' ,label= "EDSR+MSA")

plt.plot(axis, loss_r_log.numpy(),color = 'cyan' ,label= "EDSR+RAT" )
plt.plot(axis, loss_s_log.numpy(),color = 'red' ,label= "EDSR+RAT+MSA")
plt.title("Training Loss Curve")
plt.legend()
plt.xlabel("Number of Epoches")

plt.subplot(122)
plt.plot(axis, savgol_filter(psnr_log.numpy().squeeze(),5,2),color = 'black' ,label= "EDSR_baseline")
plt.plot(axis, savgol_filter(psnr_b_log.numpy().squeeze(),5,2),color = 'yellow' ,label= "EDSR+MSA")

plt.plot(axis, savgol_filter(psnr_r_log.numpy().squeeze(),5,2),color = 'cyan' ,label= "EDSR+RAT")
plt.plot(axis, savgol_filter(psnr_s_log.numpy().squeeze(),5,2),color = 'red' ,label= "EDSR+RAT+MSA")

plt.title("PSNR on SRRAW Dataset")
plt.legend()
plt.xlabel("Number of Epoches")

plt.savefig("result_edsr.pdf",bbox_inches = 'tight',pad_inches = 0)
